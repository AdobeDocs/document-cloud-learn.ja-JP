...
このチュートリアルでは、3つの実際のワークフローを通じて、SecurFinancialのITチーム・メンバーをフォローします。 PIIを削除し、インシデントレポートからエグゼクティブサマリーを生成します。 社内セキュリティ基準に照らしてベンダー契約を分析し、フォローアップのアンケートを作成します。 次に、静的なポリシードキュメントを取得し、インタラクティブな組織全体のガバナンスリソースに変換します。従業員は、どのデバイスでもアクセスしてクエリを実行できます。 各チュートリアルでは、Acrobat StudioのAIアシスタント、PDFスペース、Adobe Expressを使用することで、手作業による作業時間を数分に短縮すると同時に、すべての手順をIT部門が管理できるようにしています。このユースケースは、順番に完了するように設計されており、各ビデオ作成は前の手順に基づいています。
