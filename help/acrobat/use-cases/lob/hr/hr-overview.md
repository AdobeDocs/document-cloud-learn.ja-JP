---
title: Acrobat HRユースケースの概要
description: 人事チームがAcrobatを使用して、従業員ライフサイクル全体の文書とワークフローを管理する方法について説明します
feature: Gen AI
role: User
level: Beginner, Intermediate
jira: KT-20534
source-git-commit: 5f9eb79aaedd0dcab16ef0483f1df7e359280b73
workflow-type: tm+mt
source-wordcount: '135'
ht-degree: 1%

---

# Acrobat人事ユースケースの概要

人事チームがAcrobatを使用して、従業員ライフサイクル全体の文書とワークフローを管理する方法について説明します。 複雑な人事資料の理解や関係者との共同作業から、ブランド上の明確なコンテンツの作成に至るまで、Acrobatを使用して人事チームがより効率的に作業し、一貫性を保ち、より良い社員体験を提供する方法をご覧ください。

## 新機能

>[!BEGINTABS]

>[!TAB インサイトからインパクトまで：人事ワークフローの合理化]

人事チームがAcrobat Studioを使用して、従業員のオンボーディングコンテンツを[インサイト、コラボレーション、オンブランド配信](hr-insights.md)へと効率化する方法について説明します。

>[!ENDTABS]

## 人事ユースケースのチュートリアル

<table style="table-layout:fixed">
<tr>
  <td>
    <a href="hr-insights.md">
      <img alt="インサイトからインパクトへ：人事ワークフローの合理化" src="../../../assets/use-case-hr-insights.png" />
    </a>
    <div>
    <a href="hr-insights.md"><strong>インサイトからインパクトまで：人事ワークフローの合理化</strong></a>
    </div>
    <em>人事チームがAcrobat Studioを使用して、最初から最後まで従業員のオンボーディングを効率化する方法を説明します</em>
    <br>
  </td>
  <td>
        <img alt="スペーサー" src="../../../assets/Whitespacer.png" />
        <div>
        <br>
  </td>
  <td>
        <img alt="スペーサー" src="../../../assets/Whitespacer.png" />
        <div>
        <br>
  </td>
  <td>
        <img alt="スペーサー" src="../../../assets/Whitespacer.png" />
        <div>
        <br>
  </td>
</tr>
</table>
